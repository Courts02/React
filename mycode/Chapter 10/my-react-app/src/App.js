...
// --- Components ---
// PersonItem receives `person` as a prop and uses internal state `showAge`

function PersonItem({ person }) {
  const [showAge, setShowAge] = useState(false); // Local state to toggle age display
...
// AddPersonForm receives `onAdd` as a prop and uses internal state for form inputs
function AddPersonForm({ onAdd }) {
  const [name, setName] = useState(""); // Local state for name input
  const [age, setAge] = useState(""); // Local state for age input
  ...
  const handleSubmit = (e) => {
    ...
    onAdd({ id: generateId(), name: name.trim(), age: Number(age) }); // onAdd is the prop
